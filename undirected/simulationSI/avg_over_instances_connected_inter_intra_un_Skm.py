...
lower, upper = mean_nconnected * (1-0-tol), mean_nconnected * (1.0+tol)
...
for f in selected_files:
    ...
    mean_Skm_list.append(d["mean_Skm"])
    var_Skm_list.append(d["var_Skm"])
...
mean_Skm_arr = np.array(mean_Skm_list)
var_Skm_arr = np.array(var_Skm_list)
...
mean_Skm = mean_Skm_arr.mean(axis=0)
...
var_Skm = var_Skm_arr.mean(axis=0) + mean_Skm_arr.var(axis=0, ddof=1)
...
out_file = curves_dir / f"curves_average_filtered_inter_intra_Skm_{simID}_N{Nv}_k{k}.npz"
np.savez_compressed(
    out_file,
