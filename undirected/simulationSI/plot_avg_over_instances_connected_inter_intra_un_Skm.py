...
simID = "SIsimUNDIRECTED20260424140333" #SIsim20260401112526
Nv = "20"
k = "2"
...
mean_Skm_cut = mean_Skm[mask]
std_Skm_cut = std_Skm[mask]
...
print('CHECK: ', np.allclose(mean_Skm_cut.sum(axis=(1,2)), mean_fractions_cut[:,0]))
# -------------------
...
T, Kmax, Mmax = mean_Skm_cut.shape

for k in range(Kmax):
    for m in range(Mmax):
        y = mean_Skm_cut[:, k, m]

        # skip empty curves (important!)
        if np.all(y == 0):
            continue

        plt.plot(time_grid_cut, y, alpha=0.5, label=f"k={k}, m={m}")

plt.xlabel("Time")
plt.ylabel("S_{k,m}")
plt.title("All S_{k,m}(t)")
...
S_from_fractions = mean_fractions_cut[:, 0]
S_from_Skm = mean_Skm_cut.sum(axis=(1, 2))

diff = S_from_fractions - S_from_Skm

plt.figure(figsize=(8, 5))

plt.plot(time_grid_cut, S_from_fractions, label="S from fractions (state 0)")
plt.plot(time_grid_cut, S_from_Skm, "--", label="S from sum S_km")

plt.xlabel("Time")
plt.ylabel("S count (normalized)")
plt.title("Check: S consistency")
plt.legend()
plt.tight_layout()
plt.show()

plt.figure(figsize=(8, 4))
plt.plot(time_grid_cut, diff, color="black")
plt.axhline(0, linestyle="--", linewidth=1)
plt.title("Difference: fractions - Skm sum")
plt.xlabel("Time")
plt.ylabel("Error")
plt.tight_layout()
plt.show()
