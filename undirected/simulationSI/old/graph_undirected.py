...
def generate_undirected_ER_graph_sorted(N_vertices, p_edges):
    ...
    # Step 3: sort edges lexicographically by source, then target
    order = np.lexsort((v2, v1))
    v1_sorted = v1[order]
...
def v1_pointer(v1_sorted, N_vertices): 
    ...
    return ptr #v2_of_source_i = v2_sorted_by_v1[ptr[i]:ptr[i+1]]

def sort_by_v2(v1_sorted, v2_sorted_by_v1):
    E = len(v1_sorted)
    edge_ids = np.arange(E)

    idx_sort_v2 = np.lexsort((v1_sorted, v2_sorted_by_v1))

    v1_sorted_by_v2 = v1_sorted[idx_sort_v2]
    v2_sorted = v2_sorted_by_v1[idx_sort_v2]
    edge_ids_sorted_by_v2 = edge_ids[idx_sort_v2]
...
def take_largest_connected_component(N_vertices_full, v1, v2):
    ...
    N_vertices = int(mask_vertices.sum())
    ...
    old_to_new[mask_vertices] = np.arange(N_vertices, dtype=np.int32)
    ...
    return N_vertices, v1_new, v2_new
